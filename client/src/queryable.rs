...
impl<C: RpcApi> Queryable<C> for luckycoin::blockdata::block::Block {
    type Id = luckycoin::BlockHash;

    fn query(rpc: &C, id: &Self::Id) -> Result<Self> {
        let rpc_name = "getblock";
        let hex: String = rpc.call(rpc_name, &[serde_json::to_value(id)?, 0.into()])?;
        let bytes: Vec<u8> = luckycoin::hashes::hex::FromHex::from_hex(&hex)?;
        Ok(luckycoin::consensus::encode::deserialize(&bytes)?)
    }
}

impl<C: RpcApi> Queryable<C> for luckycoin::blockdata::transaction::Transaction {
    type Id = luckycoin::Txid;
